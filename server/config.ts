...
export default [
  ...
  config(CACHE_STORE_CFG, ({ env }) => ({
    ...
    // При деплое в CACHE_SIGNATURE можно подставить хэш комита
    signature: `${env.CACHE_SIGNATURE || 'some6'}${env.PROD ? 'P' : 'D'}`,
    ...
    rules: [
      {
        ...
        ssrWait: env.DEV,
        // Отправлять старый кэш (true) или SPA (false) если ssrWait=false
